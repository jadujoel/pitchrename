...
class Bark:
    ...
    def fft2barkmx_rasta(self):
        ...
        # bark per filt
        self.step_barks = nyqbark//(nfilts-1)

        # Frequency of each FFT bin in Bark
        self.binbarks = self.hz2bark(np.linspace(0,(nfft//2),(nfft//2)+1)*fs//nfft)

        for i in range(nfilts):
          f_bark_mid = min_bark + (i)*self.step_barks
          # Linear slopes in log-space (i.e. dB) intersect to trapezoidal window
          lof = np.add(self.binbarks, (-1*f_bark_mid - 0.5))
          hif = np.add(self.binbarks, (-1*f_bark_mid + 0.5))
